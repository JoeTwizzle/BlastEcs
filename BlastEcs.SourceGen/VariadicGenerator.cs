...
public class VariadicGenerator : IIncrementalGenerator
{
    ...
    // For now, since this is limited to BlastEcs, it's enough to use this.

    /// <summary>
    /// Stores useful info about a member marked with <see cref="SourceGen.VariadicAttribute"/>.
    /// </summary>
    private class VariadicInfo
    {
        /// <summary>
        /// The generic type to look for, as passed to <see cref="SourceGen.VariadicAttribute"/>. For example, T0.
        /// </summary>
        public string Type { get; set; } = string.Empty;

        /// <summary>
        /// The last variadic index to generate, passed into <see cref="SourceGen.VariadicAttribute"/>.
        /// </summary>
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var infos = context.SyntaxProvider.ForAttributeWithMetadataName("BlastEcs.Builtin.VariadicAttribute",
...
//namespace BlastEcs.SourceGen
//{
//    // Path: Generators/VariadicGenerator.cs
...
//                    )
//                    .Where(syntaxContext =>
//                        syntaxContext.Node != null &&
//                        syntaxContext.SemanticModel.GetDeclaredSymbol((syntaxContext.Node as MethodDeclarationSyntax)!) is IMethodSymbol methodSymbol &&
//                        methodSymbol.GetAttributes().Any(attr => attr.AttributeClass?.Name == "VariadicAttribute")
//                    );

//                context.RegisterSourceOutput(methodDeclarations, (sourceProductionContext, methodSyntaxContext) =>
//                {
//                    MethodDeclarationSyntax? methodDeclaration = methodSyntaxContext.Node as MethodDeclarationSyntax;
//                    if (methodDeclaration == null)
//                        return;

//                    SemanticModel semanticModel = methodSyntaxContext.SemanticModel;
//                    IMethodSymbol? methodSymbol = semanticModel.GetDeclaredSymbol(methodDeclaration);

//                    var attributeData = methodSymbol?.GetAttributes().FirstOrDefault(a => a.AttributeClass?.Name == "VariadicAttribute");
//                    if (attributeData == null)
//                        return;

//                    int count = (int)attributeData.ConstructorArguments[0].Value!;
...
//                    var sourceText = GenerateVariants(classDeclaration, methodDeclaration, count);
//                    sourceProductionContext.AddSource($"{classDeclaration.Identifier.Text}_{methodDeclaration.Identifier.Text}_Variadic.g.cs", SourceText.From(sourceText, Encoding.UTF8));
//                });
//            }

//            private string GenerateVariants(ClassDeclarationSyntax classDeclaration, MethodDeclarationSyntax methodDeclaration, int count)
//            {
//                var className = classDeclaration.Identifier.Text;
//                var methodName = methodDeclaration.Identifier.Text;
//                var returnType = methodDeclaration.ReturnType.ToString();
//                var parameterList = methodDeclaration.ParameterList.ToString();

//                var generatedMethods = new List<string>();
//                for (int i = 1; i <= count; i++)
//                {
//                    var genericTypes = string.Join(", ", Enumerable.Range(1, i).Select(n => $"T{n}"));
//                    var genericConstraints = string.Join("\n", methodDeclaration.ConstraintClauses.Select(clause => clause.ToString()));

//                    var lineInfos = ProcessLines(methodDeclaration.Body?.ToString() ?? string.Empty, i);

//                    var methodVariant = $@" 
//    public {returnType} {methodName}<{genericTypes}>({parameterList}) {genericConstraints}
//    {lineInfos}";
//                    generatedMethods.Add(methodVariant);
//                }

//                return $@"
//namespace {Utils.GetNamespace(classDeclaration)};
...
//            private IEnumerable<LineInfo> ProcessLines(string methodBody, int count)
//            {
